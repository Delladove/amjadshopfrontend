...
export default function Home() {
  ...
  const { data: orders = [], isPending:ordersLoading } = useQuery({ queryKey: ["orders", "billType", tab], queryFn: () => ordersApi.list({ billType: tab }) , refetchInterval:4000, refetchIntervalInBackground: false });
  const { data: allOrders = [], isPending:allOrdersLoading } = useQuery({ queryKey: ["orders"], queryFn: () => ordersApi.list(),refetchInterval:4000, refetchIntervalInBackground: false  });
